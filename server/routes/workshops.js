...
router.put('/:id', upload.single('image'), async (req, res) => {
  try {
    const { title, date, seats, description, category, price } = req.body;
    
    // Prepare object with text fields
    const updateData = { 
        title, 
        date, 
        seats, 
    ...
    await Workshop.findByIdAndUpdate(req.params.id, updateData);
  ...
  router.delete('/:id', async (req, res) => {
    try {
      // Optional: Double check for registrations here for extra security
